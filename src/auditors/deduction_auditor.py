...
def check_deduction(variables: dict) -> None:
    ...
    Reconciliation Formula:
        ...
        2. UnitRetailPrice(USD) = UnitFobPrice(USD) + UnitMerchantFreightExpense(USD) +
    ...
    unit_merchant_freight_expense = variables[vn.UNIT_MERCHANT_FREIGHT_EXPENSE]
    ...
    if not math.isclose(unit_fob_price + unit_merchant_freight_expense + unit_tariff + unit_retail_margin,
                        unit_retail_price,
                        rel_tol=settings.AUDIT_REL_TOL,
                        abs_tol=settings.AUDIT_ABS_TOL):
        raise ValueError(
            f"Reconciliation error: unit_fob_price({unit_fob_price}) "
            f"+ unit_merchant_freight_expense({unit_merchant_freight_expense}) "
...
class DeductionAuditor(Auditor):
    ...
    Description:
        ...
        2. Pricing Reconciliation: Ensures that the sum of the FOB value and all
           component costs (MerchantFreight, Tariff, Margin) equates to the defined Retail Price.

    Audit Logic:
        - Ensures: 0 < Sum(DeductionRates) < 1
        - Ensures: UnitRetailPrice == Sum(FOB, MerchantFreight, Tariff, Margin)
    """
